...
define docker::container (
  ...
  # name is always defined as title, set $named to false to use random name
  # If you use a random name you will need to set ensure => runonce
  # The service can't track a random named container
  # TODO: look into using cidfile to track un-named docker containers
  validate_bool($named)
  validate_string($net)
  ...
  $script_file = "${config_dir}/${title}.sh"
  ...
  case $ensure {
    'present': {
      ...
      file { $script_file:
        ...
        mode    => '0700',
        content => template('docker/etc/docker/docker.sh.erb'),
        require => File[$config_dir]
      }
      service { $title:
        ensure   => running,
    ...
    'runonce': {
      #Don't set the service only notify an exec
      file { $script_file:
        ensure  => file,
        mode    => '0700',
        content => template('docker/etc/docker/docker.sh.erb'),
        require => File[$config_dir],
        notify  => Exec[$script_file],
      }
      exec { $script_file:
        refreshonly => true,
        require     => [
